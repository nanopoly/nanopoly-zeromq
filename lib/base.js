...
class Base {
    ...
    constructor(publisher, subscriber, options) {
        ...
        this._interval = setInterval(
            ...
            is.number(this._options.interval) &&
                this._options.interval > minInterval
     ...
     * @param {String} address push socket's address
     * @param {Object} msg message
     * @memberof Base
     */
    send(address, msg) {
        msg._ = this._id;
        if (is.not.string(msg.id) || !shortid.isValid(msg.id))
    ...
    stop() {
        ...
        if (this._publisher) this._publisher.quit();
        if (this._subscriber) this._subscriber.quit();
        for (let address in this._push)
            if (this._push[address] instanceof Socket)
                this._push[address].disconnect();
        for (let address in this._pull)
            if (this._pull[address] instanceof Socket)
                this._pull[address].disconnect();
    }
    ...
    _parseMessage(txt) {
        ...
        try {
            ...
            else if (!this._pair[msg._])
                throw new Error(`invalid instance id: ${msg._}`);
        } catch (e) {
            msg.e = e.message;
        }
        return msg;
    ...
    _garbageCollector() {
        if (!this._progress && is.not.empty(this._pair)) {
            ...
            const now = Date.now();
            const gc =
                is.number(this._options.gc) && this._options.gc > minGC
                    ? this._options.gc
                    : minGC;
            try {
                if (now - this._gc >= gc) {
                    ...
                    const ping =
                        is.number(this._options.ping) &&
                        this._options.ping > minInterval
                            ? this._options.ping
                            : minInterval;
                    if (now - this._ping >= ping) {
                        this._ping = Date.now();
                        this._publisher.publish(
                            this._channel(`${this._name}-ping`),
    ...
    /**
     * @description creates actual channel name
