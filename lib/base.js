...
class Base {
    ...
    constructor(publisher, subscriber, options) {
        this._options = Object.assign({ log: 'error', ping: 10000, gc: 120000, interval: 3000, prefix: 'npzmq' },
            ...
            try {
                if (now - this._gc >= gc) {
                    ...
                    if (now - this._ping >= ping) {
                        this._ping = Date.now();
                        this._publisher.publish(this._channel(`${ this._name }-ping`), JSON.stringify({ id: this._id }));
