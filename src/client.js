...
class Client extends Base {
        ...
        if (this._id !== msg.id) {
            if (status === 'push') {
                if (!this._pair[msg.id]) {
                    this._logger.info(
                        `push received from ${msg.id}`,
                        this._name,
                        this._id
                    );
                    const pull = new Socket('pull', msg.sock);
                    pull.handle('error', (e) =>
                        this.logger.error(e, this._name, this._id)
                    );
                    ...
                    try {
                        ...
                        this._publisher.publish(
                            this._channel('client-push'),
                            JSON.stringify({
                                id: this._id,
                                ip: this._ip,
                                port,
                                sock: push._id.split('/').pop(),
                                address: pull._address,
                            })
                        );
                    } catch (e) {
                ...
                if (this._pair[msg.id]) {
                    ...
                    this._logger.info(
                        `ping received from unknown server(${msg.id})`,
                        this._name,
                        this._id
                    );
            }
     ...
     */
    _onMessage(p) {
        this._handler(this._parseMessage(p)).catch(async (e) =>
    ...
    start(handler) {
        ...
        this._subscriber.on('message', (ch, msg) => {
            try {
                msg = JSON.parse(msg);
                if (is.object(msg))
                    this._onStateChange(ch.split('-').pop(), msg, handler);
            } catch (e) {
                this._logger.error(e, this._name, this._id);
            }
        });
        this._publisher.publish(
            this._channel('client-init'),
            JSON.stringify({ id: this._id, ip: this._ip })
        );
    }
}
